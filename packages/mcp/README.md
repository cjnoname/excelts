...
| `sheet_write`           | Create an `.xlsx` or `.xlsb` from a declarative spec. `fromCsv` pulls source data in server-side.                            |
| `sheet_edit`            | Patch an existing `.xlsx`, `.xlsm`, or `.xlsb` — cells, ranges, formulas, rows, styles, sheets. Atomic and backed up.        |
