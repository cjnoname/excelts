...
**边框是刻意缺席的，这是一个发现而非投入不足。** 全部九个由 Excel 生成的参考工作簿都只含**一条**
`BrtBorder`，且在每个文件中字节完全相同：默认的"无边框"项。一个 51 字节结构的单一样本只能确立
它有 51 字节，别无其他 —— 没有任何一条边、线型或颜色被使用过，因此无从读取。相比之下字体在五种
语言的十五个样本中各不相同，这才是它能做的原因。
...
- 公式中的数组常量、结构化引用，以及整行/整列引用。每一种都按名字被拒绝，而不是被编码成别的东西。
- 共享公式：读取时报告 `PtgExp` 延迟引用，写入时拒绝。
- 边框。见上文的发现：语料只确立了该记录的大小，没有确立它的任何一个字段。
- 冻结与拆分窗格（`BrtPane`）、表格、自动筛选、数据验证、条件格式、分页符、批注、形状、图表、
  迷你图、表单控件、已配置的工作表保护、打印区域与打印标题。它们的记录在任何参考工作簿中都没有
  出现，因此布局尚未确立 —— 而本模块不猜偏移。确立它们需要一个由 Excel 生成、真正用到该特性的
  `.xlsb`；`DOCUMONSTER_XLSB_CORPUS_DIR` 就是放这种文件的地方。
...
- 有七条单元格记录——`BrtShortBlank`、`BrtShortRk`、`BrtShortError`、`BrtShortBool`、
  `BrtShortReal`、`BrtShortSt`、`BrtShortIsst`——没有已确立的载荷布局。它们在**九个**由 Excel
  生成的参考工作簿中一个都没出现，全部使用完整的 `BrtCell*` 形式，因此实际缺口远比它们的名气要
  窄。reader 会识别、计数并报告它们，而不是猜测偏移或丢弃单元格；writer 则从不发射它们。
  `spec.test.ts` 会断言清单上的每个名字确实是「无已声明布局的 cell 记录」，因此这个缺口不能靠删清单来关闭。
...
十一个 `.xlsb` 文件,其中**九个是 Excel 自己的输出**,另两个 —— `issue_666_lost_sheets` 与
`issue_666_panic` —— 是手工削减的 bug 复现文件。本模块的每一处布局断言都建立在那九个之上;两个削减
文件不能佐证任何九个文件尚未确立的事,而这个区分很重要,因为这里的整套方法就是「从 Excel 的输出读出
来,而不是假设」。
...
二进制；把 `DOCUMONSTER_XLSB_CORPUS_DIR` 指向一个存放 `.xlsb` 的目录，即可用真实文件跑同一批检查。
